...
export function scanDocument(e: any) {
  ...
  if (!doc) return createMessageCard(t("error"), t("noDoc"));
  ...
  for (let i = 0; i < numChildren; i++) {
    ...
    if (
      ...
      while ((match = regex.exec(text)) !== null) {
        ...
        // Assign to people via config (supports aliases)
        ...
        assignees.forEach((assignee) => {
          ...
          if (isBold) openTasks.push(taskObj);
          else completedTasks.push(taskObj);
        });
  ...
  if (
    ...
    (openMatches.length === 0 && completedMatches.length === 0)
  ) {
    return createMessageCard(t("noTasksTitle"), t("noTasksHint"));
  }
  ...
  builder.setHeader(CardService.newCardHeader().setTitle(t("scanResults")));

  // --- SECTION: OPEN TASKS ---
  let openSection = CardService.newCardSection().setHeader(
    t("openAP", { count: openTasks.length }),
  );

  if (openTasks.length === 0) {
    openSection.addWidget(
      CardService.newTextParagraph().setText(`<i>${t("noOpen")}</i>`),
    );
  } else {
    openTasks.forEach((task) => {
      let dateText = task.date
        ? ` <font color="#888888"><i>[${t("due")}: ${task.date}]</i></font>`
        : "";
      openSection.addWidget(
        CardService.newTextParagraph().setText(
          `<b>${task.person}</b>: ${task.action}${dateText}`,
        ),
      );
    });
  }
  ...
  let completedSection = CardService.newCardSection()
    .setHeader(t("completedAP", { count: completedTasks.length }))
    .setCollapsible(true)
    .setNumUncollapsibleWidgets(0); // 0 means everything hides under a "Show More" toggle

  if (completedTasks.length === 0) {
    completedSection.addWidget(
      CardService.newTextParagraph().setText(`<i>${t("noCompleted")}</i>`),
    );
  } else {
    completedTasks.forEach((task) => {
      let dateText = task.date ? ` <i>[${t("due")}: ${task.date}]</i>` : "";
      // Use strikethrough <s> and grey coloring for completed tasks
      completedSection.addWidget(
        CardService.newTextParagraph().setText(
          `<font color="#999999"><s><b>${task.person}</b>: ${task.action}</s>${dateText}</font>`,
        ),
      );
    });
  }
  ...
  if (isTodoistEnabled && openTasks.length === 0) {
    actionSection.addWidget(
      CardService.newTextParagraph().setText(`<i>${t("noOpenToSync")}</i>`),
    );
  }
  ...
  if (openMatches.length > 0 || completedMatches.length > 0) {
    ...
    actionSection.addWidget(
      CardService.newTextButton()
        .setText(t("applyDocChanges"))
        .setTextButtonStyle(CardService.TextButtonStyle.FILLED)
        .setOnClickAction(applyAction),
    );
  }

  const foundPeopleList = Object.keys(foundPeople).filter(
  ...
  if (!isTodoistEnabled) {
    actionSection.addWidget(
      CardService.newTextParagraph().setText(`<i>${t("todoistDisabled")}</i>`),
    );
  }

  let backAction = CardService.newAction().setFunctionName(
    "onActionPointsHomepage",
  );
  actionSection.addWidget(
    CardService.newTextButton().setText(t("back")).setOnClickAction(backAction),
  );
